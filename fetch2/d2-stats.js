...
.d2-sum-badge{font-size:13px;font-weight:700;font-family:var(--mono,monospace);color:var(--accent,#00d4ff);background:rgba(0,212,255,0.08);border:1px solid rgba(0,212,255,0.18);border-radius:20px;padding:2px 10px;white-space:nowrap;align-self:flex-start;margin-top:1px;}
...
function initD2Stats(){
  ...
  scopes.forEach(scope => {
    ...
    d2RenderDoughnut(scope, allRides);
...
function d2RenderMainChart(scope, rides){
  ...
  const dpKm  = _d2FillMap(rides,dmKm,'km');
  const dpHm  = _d2FillMap(rides,dmHm,'hm');
  const dpCal = _d2FillMap(rides,dmCal,'cal');
  _d2MakeBarChart(scope.querySelector('.d2-main-chart'), lKm,  dpKm,  'rgba(0,212,255,');
  _d2MakeBarChart(scope.querySelector('.d2-hm-chart'),   lHm,  dpHm,  'rgba(255,61,107,');
  _d2MakeBarChart(scope.querySelector('.d2-cal-chart'),  lCal, dpCal, 'rgba(255,145,0,');
  // Update sum badges
  const sumKm  = dpKm.reduce((a,v)=>a+v,0);
  const sumHm  = dpHm.reduce((a,v)=>a+v,0);
  const sumCal = dpCal.reduce((a,v)=>a+v,0);
  const setSum = (sel, val, unit) => {
    scope.querySelectorAll(sel).forEach(el => { el.textContent = val > 0 ? val + ' ' + unit : '—'; });
  };
  setSum('.d2-sum-km',  sumKm.toFixed(1),        'km');
  setSum('.d2-sum-hm',  Math.round(sumHm)+'',    'hm');
  setSum('.d2-sum-cal', Math.round(sumCal)+'',   'kcal');
}
...
function d2RenderDoughnut(scope, rides){
  ...
  if(!svg) return;
  const s=rides.filter(r=>r.km<10).length, m=rides.filter(r=>r.km>=10&&r.km<=20).length, l=rides.filter(r=>r.km>20).length;
  const total = s+m+l;
  const set = (sel, v) => { const el=scope.querySelector(sel); if(el) el.textContent=v; };
